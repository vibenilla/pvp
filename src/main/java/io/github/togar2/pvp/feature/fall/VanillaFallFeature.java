...
public class VanillaFallFeature implements FallFeature, RegistrableFeature {
    ...
    public void init(EventNode<EntityInstanceEvent> node) {
        ...
        node.addListener(EntityTickEvent.class, event -> {
            ...
            var previousPosition = livingEntity.getPreviousPosition();
    ...
    public void handleFallDamage(LivingEntity entity, Pos currPos, Pos newPos, boolean onGround) {
        var dy = newPos.y() - currPos.y();
        var fallDistance = this.getFallDistance(entity);
        ...
        var landingPos = this.getLandingPos(entity, newPos);
        var block = entity.getInstance().getBlock(landingPos);
        ...
        if (entity.hasTag(EXTRA_FALL_PARTICLES) && entity.getTag(EXTRA_FALL_PARTICLES) && fallDistance > 0.0) {
            var position = landingPos.asVec().apply(Vec.Operator.FLOOR).add(0.5, 1, 0.5);
            var particleCount = (int) Math.max(0, Math.min(200, 50 * fallDistance));
        ...
        var safeFallDistance = entity.getAttributeValue(Attribute.SAFE_FALL_DISTANCE);
        if (effectiveFallDistance > safeFallDistance) {
            if (!block.air()) {
                var damageDistance = Math.floor(effectiveFallDistance + 1.0E-6 - safeFallDistance);
                var particleMultiplier = Math.min(0.2 + damageDistance / 15.0, 2.5);
                var particleCount = (int) (150 * particleMultiplier);
        ...
        var damage = this.getFallDamage(entity, effectiveFallDistance, damageModifier);
        if (damage > 0) {
            this.resetCurrentImpulseContext(entity);
    ...
    protected Point getLandingPos(LivingEntity livingEntity, Pos position) {
        var offset = position.add(0, -0.2, 0);
        var instance = livingEntity.getInstance();

        if (instance == null) return offset;
        if (!instance.getBlock(offset).air()) return offset;

        var offsetDown = offset.add(0, -1, 0);
        var block = instance.getBlock(offsetDown);
