...
public class VanillaBlockFeature implements BlockFeature {
    ...
    public boolean applyBlock(LivingEntity entity, Damage damage) {
        ...
        var hand = ((LivingEntityMeta) entity.getEntityMeta()).getActiveHand();
        ...
        var damageBlockEvent = new DamageBlockEvent(entity, amount, resultingDamage, false);
        ...
        var shieldDamage = this.computeShieldItemDamage(blocksAttacks, blockedDamage);
        if (shieldDamage > 0) {
        ...
        var info = DamageTypeInfo.of(damage.getType());
    ...
    protected void takeShieldHit(LivingEntity entity, LivingEntity attacker, @Nullable BlocksAttacks blocksAttacks, boolean applyKnockback) {
        ...
        var blockerStrength = (float) (BLOCK_KNOCKBACK * (1.0 - entity.getAttributeValue(Attribute.KNOCKBACK_RESISTANCE)));
