...
public final class VanillaEnvironmentDamageFeature implements EnvironmentDamageFeature, RegistrableFeature {
    ...
    private void handleDrowning(LivingEntity entity) {
        ...
        var airSupply = entity.hasTag(AIR_SUPPLY) ? entity.getTag(AIR_SUPPLY) : MAX_AIR_SUPPLY;
    ...
    private void handleDolphinMoistness(Instance instance, LivingEntity entity) {
        ...
        var moistness = entity.hasTag(DOLPHIN_MOISTNESS) ? entity.getTag(DOLPHIN_MOISTNESS) : DOLPHIN_MOISTNESS_MAX;
        moistness--;
    ...
    private void handleBlockContactDamage(LivingEntity entity) {
        ...
        var minX = (int) Math.floor(position.x() - boundingBox.width() / 2);
        var maxX = (int) Math.floor(position.x() + boundingBox.width() / 2);
        var minY = position.blockY();
        var maxY = (int) Math.floor(position.y() + boundingBox.height());
        var minZ = (int) Math.floor(position.z() - boundingBox.depth() / 2);
        var maxZ = (int) Math.floor(position.z() + boundingBox.depth() / 2);
    ...
    private void handleBerryBushDamage(LivingEntity entity, Block block) {
        ...
        var position = entity.getPosition();
        var previousPosition = entity.getPreviousPosition();
        var movementX = Math.abs(position.x() - previousPosition.x());
        var movementZ = Math.abs(position.z() - previousPosition.z());
    ...
    private void handleFreezeDamage(LivingEntity entity) {
        ...
        var freezeTicks = entity.hasTag(FREEZE_TICKS) ? entity.getTag(FREEZE_TICKS) : 0;
        var inPowderSnow = this.isInPowderSnow(instance, entity);
        var canFreeze = this.canFreeze(entity);
    ...
    private @Nullable Block findBlockInside(LivingEntity entity, Predicate<Block> predicate) {
        ...
        var minX = (int) Math.floor(position.x() + boundingBox.minX() + 1.0E-5);
        var maxX = (int) Math.floor(position.x() + boundingBox.maxX() - 1.0E-5);
        var minY = (int) Math.floor(position.y() + boundingBox.minY() + 1.0E-5);
        var maxY = (int) Math.floor(position.y() + boundingBox.maxY() - 1.0E-5);
        var minZ = (int) Math.floor(position.z() + boundingBox.minZ() + 1.0E-5);
        var maxZ = (int) Math.floor(position.z() + boundingBox.maxZ() - 1.0E-5);
    ...
    private boolean isInRain(Instance instance, LivingEntity entity) {
        ...
        var topBlockY = CoordConversion.globalToBlock(position.y() + boundingBox.maxY());
    ...
    private boolean isInWaterCauldron(Instance instance, LivingEntity entity) {
        ...
        var level = Integer.parseInt(levelProperty);
        var contentHeight = position.blockY() + (6.0 + level * 3.0) / 16.0;
        var feetY = position.y() + entity.getBoundingBox().minY();
    ...
    private boolean isEyeInWater(LivingEntity entity) {
        ...
        var eyeBlockX = entity.getPosition().blockX();
        var eyeBlockY = (int) Math.floor(entity.getPosition().y() + entity.getEyeHeight());
        var eyeBlockZ = entity.getPosition().blockZ();
    ...
    private int decreaseAirSupply(LivingEntity entity, int currentSupply) {
        var oxygenBonus = entity.getAttributeValue(Attribute.OXYGEN_BONUS);
